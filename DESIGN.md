...
- `<hp-cell variant="anchor|action|secondary|utility">` — interactive labelled hex. Cursor pointer, hue-swaps to `secondary` on hover / focus-visible / `aria-pressed`. `filled` boolean (`action` or `anchor`) for the solid-primary paint: action+filled is the high-emphasis CTA (darkens to primary-container on hover/focus for keyboard distinction); anchor+filled is the identity / grip / drag-handle variant — hover/focus/aria-pressed swap the entire fill+stroke+label to secondary, matching the unfilled anchor hue-swap motion. Used by `<hp-cluster layout="honeycomb">` as the cluster centre.
...
- **`<hp-grid>`** owns the axial coordinate system, the slot occupancy map, and the drag-overlay rendering. Children placed inside it declare a `slot` attribute (`q,r`) and a `footprint` attribute (relative offsets). The grid is the source of truth for layout; modules don't position themselves.
  - **`layout="masonry"`** opt-in mode for surfaces that don't author per-child `q`/`r`: on first render and on `pack()` calls, the grid greedy bin-packs every direct child at the leftmost-topmost axial position whose filled hexes are at hex-distance ≥ 2 from every already-placed hex (the "1-hex gap" rule). Children publish their actual filled cells via `data-fill-cells="q,r q,r …"` (`<hp-cluster>` populates this on slotchange); children without it are treated as single-hex. The gap check uses hex-adjacency (the 6 axial-distance-1 neighbours) — not rectangular bbox padding — so neighbour clusters tuck into the rect-diagonal-but-hex-far positions a non-symmetric cluster leaves around itself (e.g. a 10-child honeycomb's empty NW corner is reachable from the NE). The scan is bounded by the host's viewport width so packing wraps to new rows naturally. Subsequent packs sort children by their current `(r, q)` so dragged items stay roughly where the user put them; the grid does not auto-repack on resize or slotchange — only on explicit `.pack()` or attribute toggle. Use for component / cluster index pages where the layout's job is "fit everything tightly with a uniform gap".
