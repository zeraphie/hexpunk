...
- **Indeterminate loading for determinate operations.** When the percent is knowable, pass the real `value` — native `<progress>` (linear, styled by elements.css) or `<hp-loader value="…">` (radial). The spinning wave is the fallback for unknowable waits, not the default.
